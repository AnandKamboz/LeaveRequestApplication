    ...
    <div class="card shadow-lg">
        ...
        <div class="card-body">
            <form id="userForm" enctype="multipart/form-data" method="post"
                action="{{ route('admin.leave-types.update', $leaveType->secure_id) }}">
                @csrf
                @method('put')
                <div class=" row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="leave_type" id="leave_type" class="form-control"
                            placeholder="Enter Leave Type" value="{{ $leaveType->leave_type }}">
                        @error('leave_type')
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="max_days" id="max_days" class="form-control"
                            placeholder="Enter Max Days" value="{{ $leaveType->max_days }}">
                        @error('max_days')
                        <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <textarea name="description" id="description" class="form-control"
                            placeholder="Enter Description">{{ $leaveType->description }}</textarea>
                        @error('description')
                        <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="text-center">
                    <button type="submit" class="btn submit-btn mt-3">Update</button>
                </div>
            </form>
        </div>
    </div>
